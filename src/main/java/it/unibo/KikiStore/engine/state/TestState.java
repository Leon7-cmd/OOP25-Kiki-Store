...
public class TestState implements GameState {
    ...
    public TestState(final InputHandler input) {
        this.input = input;

        // --- 1. RESOURCE LOADING ---
        this.visualGrid = MapLoader.loadMap("maps/test.txt");
        final int[][] maskGrid = MapLoader.loadMap("maps/col/testCol.txt");

        // --- 2. MODEL INITIALIZATION ---
        final GameTile collisionMap = new TileMapImpl(maskGrid, 64);
        final CollisionHandler collisionHandler = new CollisionHandler(collisionMap);
    ...
    public void render(final GraphicsContext gc) {
        ...
        // --- WORLD RENDERING ---
        final MapRenderData mapData = new MapRenderData(visualGrid, 64);
